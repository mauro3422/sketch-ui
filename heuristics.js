// heuristics.js - layout heuristics minimas para el arbol de contencion y orientacion
...
export function buildContainmentTree(state, toGridBBox){
  ...
  // Ordenamos por area descendente para anidar de grande a chico
  rects.sort((a,b)=> (b.bb.w*b.bb.h) - (a.bb.w*a.bb.h));
  // Funcion de contencion estrica (dentro de bbox)
...
export function inferOrientationForChildren(node){
  ...
  // Heuristica: comparar dispersion en eje X vs Y usando centroides
  ...
  if (varY > varX * 1.2) return 'vertical';    // mas separados en Y -> stack vertical
  if (varX > varY * 1.2) return 'horizontal';  // mas separados en X -> fila
