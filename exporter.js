...
export function exportJSON(state, toGridBBox, toGridPoint, inferLayout, inferComponents){
  ...
  return { meta:{ grid:{ cols:state.cols, rows:state.rows, width: canvas.width, height: canvas.height }, generatedAt:new Date().toISOString() }, widgets, layout, components };
}
...
export function exportASCIIGeneric(state, toGridBBox){
  ...
  const out = [];
  out.push(sep); out.push(center(` SKETCH ASCII (${rects.length} bloques) `, cols)); out.push(sep);
  const totalRows = state.rows; const totalCols = state.cols;
  rects.forEach((r,i)=>{
    const pctH = Math.round((r.bb.h/totalRows)*100);
    const pctW = Math.round((r.bb.w/totalCols)*100);
    out.push(`| ${String(i+1).padStart(2,'0')} • ${r.label||'bloque'}  [h:${pctH}% w:${pctW}%]`.padEnd(cols-1,' ') + '|');
  });
  out.push(sep);
  return out.join('\n');
}
...
export function exportTkHybrid(tree, state){
  ...
  function walk(node){
    ...
    panes.forEach((p, i)=>{
      ...
      if (ch.children && ch.children.length){ p.inner = walk(ch); }
    });
